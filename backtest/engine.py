...
def run_backtest(
    ...
    for sym, df in tqdm(data.items(), desc="Scan signals", leave=False):
        ...
        for d in df.index[mask]:
            ...
            if burns_mode_active:
                ...
                # provisional ADX for gating
                try:
                    adx_now = float(row.get("ADX14", np.nan)) if "ADX14" in row.index else float("nan")
                except Exception:
                    adx_now = float("nan")
                ...
                if not fundamentals_ok:
                    ...
                    reason = "pass"
                    accept = True
                # Enhanced breakout extension gate (Burns mode)
                if accept and setup_type == "breakout":
                    gate_cfg = ((cfg.get("trading", {}) or {}).get("breakout_extension_gate", {}) or {})
                    if bool(gate_cfg.get("enabled", True)):
                        try:
                            ema20_val = float(row.get("EMA20", np.nan)); close_val = float(row.get("Close", np.nan))
                            ext_pct_tmp = ((close_val - ema20_val)/ema20_val) if (np.isfinite(ema20_val) and ema20_val>0 and np.isfinite(close_val)) else np.nan
                        except Exception:
                            ext_pct_tmp = np.nan
                        atr_pct_now = float(row.get("ATR_PCT", np.nan)) if "ATR_PCT" in row.index else np.nan
                        adx_now_loc = float(row.get("ADX14", np.nan)) if "ADX14" in row.index else np.nan
                        hard_thr = float(gate_cfg.get("hard_ema20_dist_pct", gate_cfg.get("max_ema20_dist_pct", 0.065)))
                        soft_thr = float(gate_cfg.get("soft_ema20_dist_pct", max(0.0, hard_thr*0.7)))
                        weak_adx = float(gate_cfg.get("weak_adx14", 20))
                        weak_ext = float(gate_cfg.get("weak_ext_pct", soft_thr))
                        min_atr_gate = float(gate_cfg.get("min_atr_pct", 0.022))
                        req_vol_soft = bool(gate_cfg.get("require_volume_in_soft_band", False))
                        req_cluster_soft = bool(gate_cfg.get("require_cluster_in_soft_band", False))
                        scale_soft = float(gate_cfg.get("risk_scale_soft_band", 0.6))
                        cluster_ok = True
                        if req_cluster_soft:
                            try:
                                scfg = (cfg.get("signals", {}) or {}).get("sr_pivots", {}) or {}
                                c_cfg = (scfg.get("cluster", {}) or {})
                                cluster_ok = bool(sr_ok) if bool(c_cfg.get("enabled", False)) else True
                            except Exception:
                                cluster_ok = True
                        vol_conf = bool(vol_ok)
                        if np.isfinite(ext_pct_tmp):
                            if (np.isfinite(adx_now_loc) and adx_now_loc < weak_adx) and (abs(ext_pct_tmp) > weak_ext):
                                reason = "breakout_extension"; accept = False
                            elif abs(ext_pct_tmp) > hard_thr:
                                reason = "breakout_extension"; accept = False
                            elif abs(ext_pct_tmp) > soft_thr and (np.isfinite(atr_pct_now) and atr_pct_now >= min_atr_gate):
                                if (req_vol_soft and not vol_conf) or (req_cluster_soft and not cluster_ok):
                                    reason = "breakout_extension"; accept = False
                                else:
                                    _soft_scale_factor = scale_soft if 0 < scale_soft < 1 else 1.0
                                    _soft_ext_pct = ext_pct_tmp
                # Pullback ADX floor (Burns mode)
                if accept and setup_type == "pullback":
                    try:
                        pb_adx_floor = float(((cfg.get("trading", {}) or {}).get("filters", {}) or {}).get("pullback_adx_min", 0) or 0)
                    except Exception:
                        pb_adx_floor = 0
                    if pb_adx_floor > 0 and np.isfinite(adx_now) and adx_now < pb_adx_floor:
                        reason = "pullback_low_adx"; accept = False
            else:
                ...
                # Breakout extension gate (non-Burns)
                if accept and setup_type == "breakout":
                    gate_cfg = ((cfg.get("trading", {}) or {}).get("breakout_extension_gate", {}) or {})
                    if bool(gate_cfg.get("enabled", True)):
                        try:
                        ...
                        if req_cluster_soft:
                            try:
                                scfg = (cfg.get("signals", {}) or {}).get("sr_pivots", {}) or {}
                                c_cfg = (scfg.get("cluster", {}) or {})
                                cluster_ok = True if not bool(c_cfg.get("enabled", False)) else bool(sr_ok)
                            except Exception:
                                cluster_ok = True
                        vol_conf = bool(vol_ok)
                        if np.isfinite(ext_pct_tmp):
                            if (np.isfinite(adx_now_loc) and adx_now_loc < weak_adx) and (abs(ext_pct_tmp) > weak_ext):
                                ...
                                if (req_vol_soft and not vol_conf) or (req_cluster_soft and not cluster_ok):
                                    ...
                                    _soft_scale_factor = scale_soft if 0 < scale_soft < 1 else 1.0
                                    _soft_ext_pct = ext_pct_tmp
                # Pullback ADX floor (non-Burns mode)
                if accept and setup_type == "pullback":
                    try:
                        pb_adx_floor = float(((cfg.get("trading", {}) or {}).get("filters", {}) or {}).get("pullback_adx_min", 0) or 0)
                    except Exception:
            ...
            if accept:
                ...
                if '_soft_scale_factor' in locals():
                    cand['scale_risk_factor'] = float(_soft_scale_factor)
                    if '_soft_ext_pct' in locals():
                        cand['soft_ext_pct'] = float(_soft_ext_pct)
                    del _soft_scale_factor
                    if '_soft_ext_pct' in locals():
                        del _soft_ext_pct
                candidates_by_day[d].append(cand)
    ...
    for day in tqdm(calendar, desc="Backtest", leave=True):
        ...
        for pend in list(pending_markets):
            ...
            # Optional fill-day recheck for breakout over-extension (price may gap further at open)
            try:
                gate_cfg = ((cfg.get("trading", {}) or {}).get("breakout_extension_gate", {}) or {})
                if gate_cfg.get("fill_day_recheck", False):
                    hard_pct = float(gate_cfg.get("hard_ema20_dist_pct", 0.0))
                    if np.isfinite(pend.ema20_dist_pct):
                        # Recompute live distance vs EMA20 using today's open (approx) if df has EMA20
                        ema_col = "EMA20" if "EMA20" in df.columns else None
                        live_dist_pct = pend.ema20_dist_pct
                        if ema_col and day in df.index and np.isfinite(df.at[day, ema_col]) and df.at[day, ema_col] > 0:
                            live_dist_pct = abs((entry_px - float(df.at[day, ema_col])) / float(df.at[day, ema_col]))
                        if hard_pct > 0 and live_dist_pct > hard_pct:
                            # Cancel this fill; treat as extension rejection at open
                            pending_markets.remove(pend)
                            reject_reasons.append({
                                ...
                                "score": pend.score,
                                "reject_reason": "fill_day_hard_extension",
                            })
                            continue
            except Exception:
                pass
            ...
            # Apply risk scaling for soft-band breakout extensions (captured on PendingMarket)
            scale_factor = getattr(pend, "scale_risk_factor", 1.0) or 1.0
            if scale_factor <= 0:
                scale_factor = 1.0  # fail-safe
            scaled_qty_mgr = int(math.floor(qty_mgr * scale_factor))
            if qty_mgr > 0 and scaled_qty_mgr <= 0:
                scaled_qty_mgr = 1  # allow minimal probe if original sizing >0
            qty_mgr_eff = scaled_qty_mgr
            qty_bp = _qty_bp_cap(entry_px, cash, bp_multiple)
            qty = min(qty_mgr_eff, qty_bp)
            cap_reason = "full_size"
            if qty_mgr_eff < base_qty:
                cap_reason = "risk_cap"
            elif qty < qty_mgr_eff:
                cap_reason = "bp_cap"
            elif scale_factor < 0.9999:  # scaled but not otherwise capped
                cap_reason = "scaled"
            if qty <= 0:
                if pend in pending_markets:
                    pending_markets.remove(pend)
            ...
            pos_created.scale_risk_factor = float(scale_factor)
            try:
            ...
            entries_audit.append({
                ...
                "order_type": "market",
                "scale_risk_factor": float(getattr(pos_created, 'scale_risk_factor', 1.0)),
            })
            # same day exit check
            ...
            if exit_reason:
                ...
                if adapt_enabled and pos.per_share_risk > 0:
                    ...
                    pos._adaptive_shrunk = False
            week_new_count[wk] += 1
            if pend in pending_markets:  # avoid double-removal if earlier logic removed it
                pending_markets.remove(pend)
        ...
        for pend in list(pending_limits):
            ...
            if target <= 0:
                continue
            qty_mgr = rm_size_position(cfg_day, entry_px, stop)
            # Apply risk scaling (soft-band breakout extension) captured on PendingLimit
            scale_factor = getattr(pend, "scale_risk_factor", 1.0) or 1.0
            if scale_factor <= 0:
                scale_factor = 1.0
            scaled_qty_mgr = int(math.floor(qty_mgr * scale_factor))
            ...
            if pos.side == "long":
                ...
                if h >= pos.stop:
                    ...
                    exit_reason = "target_same_day"
            if exit_reason:
                # Same-day stop tolerance (optional)
                protect_cfg = (cfg.get("trading", {}) or {}).get("same_day_exit_protect", {}) or {}
                if exit_reason == "stop_same_day" and bool(protect_cfg.get("enabled", False)) and pos.per_share_risk > 0:
                    try:
                        min_adverse_R = float(protect_cfg.get("min_adverse_R", 0.0))
            ...
            if entry_type == "market":
                ...
                pending_markets.append(
                    PendingMarket(
                        ...
                        scale_risk_factor=float(sig.get("scale_risk_factor", 1.0)),
                    )
                )
